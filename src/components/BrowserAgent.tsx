...
export interface ActionTriggerItem {
  type: string;
  args: any;
  id: string;
...
interface BrowserAgentProps {
  ...
  actionTrigger?: ActionTriggerItem | null;
  onTriggerFinished?: (id: string) => void;
}
...
export const BrowserAgent: React.FC<BrowserAgentProps> = ({
  ...
  const [diagnosticStatus, setDiagnosticStatus] = useState<
    "secure" | "restricted" | "error" | "analyzing" | "blank"
  >("blank");
  ...
  const checkIsRestricted = (
    ...
    try {
      ...
      // Allow YouTube watch links, embeds, search results, and portal navigation
      ...
      if (
        ...
        return {
          restricted: true,
          reason:
            "Google security parameters prohibit embedding of credential ports and account consoles to mitigate clickjacking.",
        };
      }
      if (hostname.includes("chatgpt.com") || hostname.includes("openai.com")) {
        return {
          restricted: true,
          reason:
            "OpenAI requires secure browser authentication checks, cloudflare protections, and user-token cookie contexts.",
        };
      }
      if (
        ...
        return {
          restricted: true,
          reason:
            "Gmail demands authenticated, non-nested visual scopes to secure sensitive correspondence tokens.",
        };
      }
      if (hostname.includes("github.com")) {
        return {
          restricted: true,
          reason:
            "GitHub deploys 'X-Frame-Options: deny' on all repositories and workspace interfaces.",
        };
      }
      if (
        ...
        return {
          restricted: true,
          reason:
            "Social networks require active secure sessions and forbid third-party iframe frame injection.",
        };
      }
      return { restricted: false, reason: "" };
    } catch {
  ...
  useEffect(() => {
    if (initialUrl) {
      const startUrl =
        initialUrl === "about:blank" ? "about:blank" : initialUrl;
        ...
        try {
          ...
          fetch(`/api/youtube-search?q=${encodeURIComponent(q)}`)
            ...
            .then((data) => {
              ...
              // Auto-play the top result if this is a play request or song search
              const isPlayIntent =
                urlObj.searchParams.get("autoplay") === "1" ||
                q.toLowerCase().includes("song") ||
                q.toLowerCase().includes("gana") ||
                q.toLowerCase().includes("gaana") ||
                q.toLowerCase().includes("play") ||
                q.toLowerCase().includes("music") ||
                q.toLowerCase().includes("sanam");

              if (isPlayIntent && results.length > 0) {
                const firstVideo = results[0];
                navigateToUrl(
                  `https://youtube.com/watch?v=${firstVideo.videoId}`,
                );
              } else {
                setTabs((prev) =>
                  prev.map((t) =>
            ...
            .catch((err) => {
              ...
              setYtSearchError(
                err.message || "Failed loading real YouTube results.",
              );
              setYtSearchLoading(false);
              setTabs((prev) =>
                prev.map((t) =>
  ...
  // Read Desktop Agent server status on a loop
  useEffect(() => {
    let isMounted = true;
      ...
      try {
        ...
        if (res.ok && isMounted) {
          ...
          if (data.tools && Array.isArray(data.tools)) {
            setLocalLogs(
              data.tools.slice(0, 30).map((t: string, idx: number) => ({
                id: `${idx}-${t}`,
                text: `Agent tool ready: ${t}`,
                type: "info" as const,
              })),
            );
          }
    ...
    if (iframe && iframe.contentWindow) {
      try {
        iframe.contentWindow.onerror = (
          ...
          const errMsg = `${message} (Line ${lineno ?? 0}:${colno ?? 0}) at ${source ?? "unknown"}`;
  ...
  useEffect(() => {
    const handleNavigationMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === "NAVIGATE" && event.data.url) {
        console.log(
          "[Myraa Browser] Same-origin child iframe navigated to:",
          event.data.url,
        );
      ...
      const safeCallback = (res: any) => {
        if (!isCallbackCalled) {
          ...
          try {
            ...
            console.warn("[Voice Callback Error]:", e);
          }
          if (onTriggerFinished) {
            onTriggerFinished(actionTrigger.id);
          }
        }
      };
      ...
      try {
        switch (type) {
          case "browserOpen": {
            ...
            safeCallback({
              result: `Opening ${cleanTitle} for you now. Let me check what is there.`,
            });
            break;
          }
          case "browserSearch": {
            ...
            const isYtRelated =
              ...
              query.toLowerCase().includes("gana") ||
              query.toLowerCase().includes("gaana") ||
              query.toLowerCase().includes("play") ||
              query.toLowerCase().includes("music") ||
              query.toLowerCase().includes("sanam") ||
              query.toLowerCase().includes("marjava") ||
              (activeTab && activeTab.url.includes("youtube"));

            if (isYtRelated) {
              const cleanYtQ = query
                .replace(/youtube|search|find|play|online/gi, "")
                .trim();
              const destUrl = `https://youtube.com/results?search_query=${encodeURIComponent(cleanYtQ || query)}&autoplay=1`;
              navigateToUrl(destUrl);
              safeCallback({
                result: `Searching YouTube for "${cleanYtQ || query}" and starting playback.`,
              });
            } else {
              const searchUrl = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
              navigateToUrl(searchUrl);
              safeCallback({
                result: `Searching for "${query}" right now. Working on it.`,
              });
            }
            break;
          }
          case "browserGoBack": {
            handleBack();
            safeCallback({
              result: "Let me go back to the previous webpage for you.",
            });
            break;
          }
          case "browserTabAction": {
            ...
            if (tabAction === "new") {
              ...
              safeCallback({ result: "Opening a new browser tab now." });
            } else if (tabAction === "close") {
              const targetId = tabId || activeTabId;
              handleCloseTab(targetId);
              safeCallback({ result: "Done. Closed the browser tab." });
            } else if (tabAction === "switch") {
              if (tabId && tabs.some((t) => t.id === tabId)) {
                setActiveTabId(tabId);
                safeCallback({ result: "Let's see. Switched to that tab." });
              } else {
                safeCallback({ error: "No matching tab code found." });
              }
            }
            break;
          }
          case "browserScroll": {
            ...
            if (iframe && iframe.contentWindow) {
              ...
              safeCallback({
                result: `Working on it. Scrolled the browser view ${direction}.`,
              });
            } else {
              safeCallback({
                error: "Cannot automate scrolling on empty home tab.",
              });
            }
            break;
          }
          case "browserType": {
            ...
            if (iframe && iframe.contentWindow) {
              ...
              if (inputEl) {
                ...
                safeCallback({ result: `Typing in "${text}" for you.` });
              } else {
                safeCallback({
                  error:
                    "Could not find a secure input block to target typing.",
                });
              }
            } else {
              safeCallback({ error: "No website active to command typing." });
            }
            break;
          }
          case "browserClick": {
            ...
            // 1. If selector references a YouTube video or search result item
            if (
              selectorLower.includes("video") ||
              selectorLower.includes("result") ||
              selectorLower.includes("first") ||
              selectorLower.includes("1") ||
              selectorLower.includes("song") ||
              selectorLower.includes("item")
            ) {
              if (ytSearchResults && ytSearchResults.length > 0) {
                const firstVideo = ytSearchResults[0];
                navigateToUrl(
                  `https://youtube.com/watch?v=${firstVideo.videoId}`,
                );
                safeCallback({
                  result: `Playing "${firstVideo.title}" on YouTube now.`,
                });
                break;
              }
            }
            ...
            if (iframe && iframe.contentWindow) {
              try {
                ...
                if (element) {
                  element.click();
                  safeCallback({
                    result: `Success. Clicked the selected item.`,
                  });
                  break;
                }
              } catch (e) {
            ...
            safeCallback({
              result: `Action executed for "${selector}".`,
            });
            break;
          }
          case "browserMediaControl": {
            ...
            if (iframe && iframe.contentWindow) {
              ...
              try {
                ...
                if (doc) {
                  ...
                  if (video) {
                    ...
                    directDomSuccess = true;
                  }
                }
              } catch (domErr) {
                // Cross-origin iframe (YouTube embed) - expected, fall back to postMessage
              }

              if (!directDomSuccess) {
                // YouTube IFrame Player API postMessage controls
                let ytFunc = "";
                let ytArgs: any[] = [];
                ...
                if (
                  ...
                  ytFunc = "setVolume";
                  ytArgs = [value !== undefined ? value : 75];
                } else if (action === "skip" || action === "forward") {
                  ytFunc = "seekTo";
                  ytArgs = [30, true];
                }
                ...
                if (ytFunc) {
                  try {
                    ...
                    console.warn("[YouTube postMessage failed]:", postErr);
                  }
                }
              }
              ...
              safeCallback({ error: "Active streaming panel is empty." });
            }
            break;
          }
          default: {
            safeCallback({
              error: `Automation command ${type} is not implemented.`,
            });
          }
        }
      } catch (err: any) {
        safeCallback({ error: `Visual automation exception: ${err.message}` });
      }
    };
        ...
        prev.map((t) =>
          t.id === activeTabId
            ? {
                ...
                title: "Start Page",
                history: [
                  ...t.history.slice(0, t.currentIndex + 1),
                  "about:blank",
                ],
    ...
    if (restrictions.restricted) {
      ...
      try {
        ...
        setNetworkErrors((prev) => [
          ...prev,
          "System pop-up blocker intercepted redirection search.",
        ]);
      }
    }
  };
  ...
  const getRenderUrl = (urlStr: string) => {
    ...
    const ytIdMatcher = urlStr.match(
    ...
    if (
      urlStr.includes("youtube.com/results") ||
      urlStr === "https://youtube.com" ||
    ...
    if (activeTab) {
      ...
      try {
        ...
        if (iframe && iframe.contentDocument) {
          ...
          if (
            bodyTxt.includes("Myraa Web Proxy Error") ||
            bodyTxt.includes("Failed loading remote website")
          ) {
            setDiagnosticStatus("error");
            setDiagnosticReason(bodyTxt);
            setNetworkErrors((prev) => [
              ...prev,
              "Proxy server failed to resolve target host.",
            ]);
          }
        }
      } catch (err) {
        ...
        <div className="relative z-10 px-4 pt-4 pb-1 border-b border-white/5 bg-slate-950/70 flex items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3 pb-2.5">
            ...
            >
              <Terminal
                size={12}
                className={showDebugPanel ? "text-amber-400 animate-pulse" : ""}
              />
        ...
        <div className="relative z-10 flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-[#07070c] relative">
            {activeTab?.url === "about:blank" ? (
              ...
              <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center space-y-8 select-none text-center scrollbar-none">
                ...
                >
                  ...
                  <div>
                    <h3 className="text-base font-bold font-mono tracking-widest text-[#f5f3ff] uppercase flex items-center justify-center gap-1.5">
                      ...
                      Holographic Navigation Projection
                    </h3>
                    <p className="text-[10px] text-slate-500 font-mono tracking-wide mt-1 leading-normal max-w-sm mx-auto uppercase">
                      Pristine Real-time web browser stream with fully
                      integrated sandbox diagnostics wrapper!
                    </p>
                  </div>
                </motion.div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3.5 w-full max-w-2xl text-left">
                  {[
                    {
                      name: "YouTube",
                      desc: "Interactive media search and embed watcher",
                      url: "https://youtube.com",
                      icon: <Play size={13} />,
                      color:
                        "text-red-400 bg-red-950/10 border-red-500/20 hover:border-red-500/40",
                    },
                    {
                      name: "Wikipedia",
                      desc: "Unlock endless global articles and content",
                      url: "https://wikipedia.org",
                      icon: <BookOpen size={13} />,
                      color:
                        "text-emerald-400 bg-emerald-950/10 border-emerald-500/20 hover:border-emerald-500/40",
                    },
                    {
                      name: "Google Search",
                      desc: "External redirect search result portal",
                      url: "https://google.com",
                      icon: <Search size={13} />,
                      color:
                        "text-blue-400 bg-blue-950/10 border-blue-500/20 hover:border-blue-500/40",
                    },
                    {
                      name: "ChatGPT Console",
                      desc: "Direct external artificial dialogue link",
                      url: "https://chatgpt.com",
                      icon: <Sparkles size={13} />,
                      color:
                        "text-sky-400 bg-sky-950/15 border-sky-500/20 hover:border-sky-500/40",
                    },
                    {
                      name: "Gmail Portal",
                      desc: "Private inbox account authentication",
                      url: "https://gmail.com",
                      icon: <Layers size={13} />,
                      color:
                        "text-amber-500/80 bg-amber-950/15 border-amber-500/20 hover:border-amber-500/40",
                    },
                    {
                      name: "DuckDuckGo Proxy",
                      desc: "Lightweight search indexes in nested viewport",
                      url: "https://duckduckgo.com",
                      icon: <Shield size={13} />,
                      color:
                        "text-orange-400 bg-orange-950/15 border-orange-500/20 hover:border-orange-500/40",
                    },
                  ].map((shortcut, i) => (
                    <motion.div
                      key={shortcut.name}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="p-1 rounded-lg bg-white/5 text-xs text-slate-300">
                          {shortcut.icon}
                        </span>
                        <ArrowRight
                          size={11}
                          className="opacity-0 group-hover:opacity-100 text-slate-300 transition duration-200"
                        />
                      </div>
                      <div>
                        ...
                        <p className="text-[9px] text-slate-400 font-mono mt-0.5 leading-normal max-w-[170px] truncate">
                          {shortcut.desc}
                        </p>
                      </div>
                    </motion.div>
                ...
                <div className="p-3 border border-white/5 bg-slate-950/50 rounded-2xl max-w-md w-full">
                  <p className="text-[9px] font-mono text-slate-500 leading-normal uppercase">
                    🌸 COMPANION TIP: Voice command Myraa to trigger any
                    automation: <br />
                    <span className="text-indigo-400 font-bold">
                      &ldquo;Search YouTube for Naruto opening&rdquo;
                    </span>
                  </p>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-md mx-auto">
                      Myraa automatically detected secure sandbox constraints
                      for{" "}
                      <span className="font-mono text-indigo-300 font-semibold">
                        {getCleanTitleFromUrl(activeTab?.url || "")}
                      </span>
                      . To bypass iframe restrictions and secure your cookies,
                      we loaded this real website in a native browser tab!
                    </p>
                  </div>
                  ...
                  <div className="p-4 rounded-xl border border-white/5 bg-slate-950/40 text-[10px] font-mono text-left space-y-1 text-slate-400">
                    ...
                    <div className="flex justify-between pt-1">
                      ...
                      <span className="text-amber-400">{diagnosticReason}</span>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-sm mx-auto font-medium">
                      Myraa was unable to load{" "}
                      <span className="font-mono text-indigo-300 font-semibold">
                        {getCleanTitleFromUrl(activeTab?.url || "")}
                      </span>{" "}
                      securely. This website may be offline, spelled
                      incorrectly, or blocking automated server access.
                    </p>
                  </div>
                  ...
                  <div className="p-4 rounded-xl border border-white/5 bg-slate-950/40 text-[10px] font-mono text-left space-y-1.5 text-slate-400 max-h-[150px] overflow-y-auto scrollbar-thin">
                    ...
                    <div className="pt-1">
                      ...
                      <span className="text-rose-400 block leading-normal">
                        {diagnosticReason ||
                          "The remote server rejected or blocked the proxy server handshake connection."}
                      </span>
                    </div>
                  </div>
              ...
              (activeTab.url.includes("youtube.com/results") ||
                activeTab.url === "https://youtube.com" ||
                activeTab.url === "https://www.youtube.com" ||
                activeTab.url === "https://youtube.com/" ||
                activeTab.url === "https://www.youtube.com/") ? (
              ...
              <div className="flex-1 w-full h-full bg-slate-950 flex flex-col overflow-hidden relative">
                ...
                <div className="px-6 py-4.5 border-b border-white/5 bg-slate-900/40 flex items-center justify-between z-10">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-slate-200 font-bold uppercase tracking-wider">
                      Real-time YouTube Portal: &ldquo;
                      {new URLSearchParams(
                        activeTab.url.substring(activeTab.url.indexOf("?")),
                      ).get("search_query")}
                      &rdquo;
                    </span>
                  </div>
                ...
                {ytSearchLoading ? (
                  <div className="flex-1 flex flex-col items-center justify-center space-y-3.5">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-red-500 animate-pulse font-extrabold">
                      Parsing Video Matrix Stream...
                    </span>
                  </div>
                ) : ytSearchError ? (
                  <div className="flex-1 flex flex-col items-center justify-center space-y-4">
                    ...
                    <p className="font-mono text-xs text-red-400 uppercase tracking-wider">
                      Search Pipeline Refused Connection
                    </p>
                    <p className="font-sans text-[11px] text-slate-500 max-w-sm text-center leading-normal">
                      {ytSearchError}
                    </p>
                    <button
                      onClick={handleRefresh}
                    ...
                    {ytSearchResults.length === 0 && (
                      <div className="col-span-full py-20 text-center space-y-2 font-mono text-slate-500 text-xs">
                        ...
                        <div className="text-[10px] text-slate-600">
                          Consider trying another video search keyword.
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex-1 w-full h-full relative overflow-hidden">
                <iframe
                  ...
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; compute-pressure; fullscreen"
                />
                    ...
                    <div className="flex-1 overflow-y-auto space-y-5 pr-1 select-text">
                      <div className="p-3.5 rounded-xl border border-indigo-500/10 bg-indigo-505/10 text-[11px] font-sans leading-normal text-indigo-300">
                        <p className="font-bold uppercase tracking-wider text-xs mb-1">
                          Local Browser Sync Mode
                        </p>
                        <span>
                          If you prefer to command a real, headed Chrome browser
                          on your desktop computer, easily launch Myraa's local
                          client script!
                        </span>
                      </div>
                      ...
                      <div className="space-y-4 font-mono text-[10px]">
                        <div>
                          <p className="text-slate-400 uppercase font-bold mb-1">
                            1. Install libraries
                          </p>
                    ...
                    <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2 pr-1 select-text scrollbar-thin">
                      {localLogs.length === 0 ? (
                        <div className="text-slate-600 italic">
                          No operations recorded yet. Speaking with Myraa will
                          generate real terminal logs.
                        </div>
                      ) : (
                        localLogs.map((log) => (
                          <div
            ...
            >
              <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-[10px] tracking-wide text-slate-400 leading-normal max-h-48 overflow-y-auto select-text scrollbar-thin">
                ...
                <div className="space-y-1 bg-white/5 p-2 rounded-xl border border-white/5">
                  <p className="text-slate-500 font-bold uppercase select-none">
                    STATE METRICS
                  </p>
                  <div>
                    <span className="text-slate-600">Frame URL:</span>{" "}
                    <span className="text-indigo-400 select-all font-semibold truncate max-w-[120px] inline-block align-bottom">
                      {activeTab?.url || "about:blank"}
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="text-slate-300">
                      {activeTab?.isLoading ? "ACTIVE FETCH" : "IDLE"}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-1 bg-white/5 p-2 rounded-xl border border-white/5 md:col-span-1">
                  ...
                  <div className="max-h-16 overflow-y-auto scrollbar-thin text-[9px] text-slate-500 select-text leading-tight">
                    {jsErrors.length === 0 ? (
                      ...
                      jsErrors.map((err, i) => (
                        <div
                          key={i}
                          className="text-rose-400 truncate border-b border-white/5 pb-1 select-text"
                        >
                          {err}
                        </div>
                      ))
