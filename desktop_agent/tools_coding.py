...
def _detect_python() -> str:
    """Return a working Python interpreter path without hardcoded user directories."""
    exe = sys.executable
    if exe and Path(exe).exists() and not exe.lower().endswith("myraa-agent.exe"):
        return exe
    candidates = [
        "python",
        "python3",
    ...
    if local_app_data:
        candidates.extend([
            os.path.join(local_app_data, "Programs", "Python", f"Python3{v}", "python.exe")
            for v in range(14, 8, -1)
        ])
    prog_files = os.environ.get("ProgramFiles", r"C:\Program Files")
    candidates.extend([
        os.path.join(prog_files, "Python", f"Python3{v}", "python.exe")
        for v in range(14, 8, -1)
    ])
    for c in candidates:
        if shutil.which(c):
