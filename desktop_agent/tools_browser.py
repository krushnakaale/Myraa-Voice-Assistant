...
@register("playYouTubeVideo")
async def play_youtube_video(args: Dict[str, Any]) -> Dict[str, Any]:
    """Search YouTube and immediately auto-play the top result in Playwright Chromium."""
    query = (
        args.get("query")
        or args.get("q")
        or args.get("name")
    ...
    if not url:
        if not query:
            raise ToolError(
                "Parameter 'query' or 'url' is required to play a YouTube video."
            )
        q = quote_plus(str(query))
    ...
    try:
        ...
        # 1. Dismiss any cookie/sign-in consent modals if present
        try:
            consent = page.locator(
                'button:has-text("Reject all"), button:has-text("Accept all"), button:has-text("I agree"), button:has-text("Dismiss"), button:has-text("No thanks"), button[aria-label*="Reject"]'
            ).first
            if await consent.is_visible(timeout=1800):
                await consent.click()
        except Exception:
            pass

        # 2. If it's a search results page, find and click the first actual video
        if "results?search_query=" in page.url or "search_query" in url:
            try:
                first_video = page.locator(
                    "ytd-video-renderer a#video-title, ytd-video-renderer a#thumbnail, ytd-item-section-renderer a#video-title"
                ).first
                await first_video.wait_for(state="visible", timeout=8000)
                await first_video.click()
            except Exception:
                first_watch = page.locator('a[href*="/watch?v="]').first
                await first_watch.click(timeout=5000)

        # 3. Ensure the video plays
        try:
            await page.wait_for_selector("video", timeout=7000)
            await page.evaluate("""() => {
        ...
        return {
            "result": f"Started playing '{query or title}' on YouTube in your browser.",
            "url": page.url,
            "title": title,
        }
    except Exception as e:  # noqa: BLE001
        raise ToolError(f"Failed to play YouTube video: {e}")
