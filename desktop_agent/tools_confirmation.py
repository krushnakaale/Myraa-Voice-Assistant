...
PERMANENT_DELETE_ACTION = "permanent_delete"
...
ACTION_LABEL = {
    ...
    PERMANENT_DELETE_ACTION: "permanently delete the file or folder",
}
...
@register("requestPermanentDelete")
def request_permanent_delete(args: Dict[str, Any]) -> Dict[str, Any]:
    """Mint a token for an irreversible deletion, bound to one path."""
    path = str(args.get("path") or "").strip()
    if not path:
        raise ToolError("A file or folder path is required for permanent deletion.")

    _purge_expired()
    token = secrets.token_urlsafe(6)
    STATE.confirmations[token] = {
        "action": PERMANENT_DELETE_ACTION,
        "target": path,
        "expires": time.time() + TOKEN_TTL_SECONDS,
    }
    return {
        ...
        "expires_in_seconds": int(TOKEN_TTL_SECONDS),
        "result": (
            f"Permanent deletion requested for {path}. Ask the user to explicitly confirm, "
            f"then call deleteFile with permanent=true and confirmation_token='{token}'."
        ),
    }


def consume_token(action: str, token: str | None, target: str | None = None) -> None:
    ...
    if entry.get("target") is not None and entry["target"] != target:
        raise ToolError(
            "Confirmation token does not match this file or folder. "
            "Ask the user again and restart the request."
        )
