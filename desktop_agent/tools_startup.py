...
def _ensure_launcher_exists() -> str:
    ...
    if os.path.isfile(path):
        return path
    # Dynamic detection of python executable
    local_app_data = os.environ.get("LOCALAPPDATA", "")
    py_candidates = [
        sys.executable if sys.executable and not sys.executable.lower().endswith("myraa-agent.exe") else "",
        "python",
        "python3",
        "py",
    ]
    ...
    py = next((p for p in py_candidates if p and _which(p)), "python")
    body = "\n".join(
        [
            "@echo off",
            "chcp 65001 >nul",
