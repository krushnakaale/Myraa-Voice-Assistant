...
export const MyraaCoreVisualizer: React.FC<MyraaCoreVisualizerProps> = ({
  ...
  const handleVideoError = (videoName: string) => {
    console.warn(
      `[Myraa Web Video] Failed to load video source for: ${videoName}`,
    );
    setHasError(true);
  };
  ...
  // Synchronized video playback state manager (smooth and flicker-free)
  useEffect(() => {
    ...
    const playVideo = async (videoEl: HTMLVideoElement | null) => {
      ...
      try {
        ...
        // Harmless browser power-save or state-interruption AbortErrors are ignored
