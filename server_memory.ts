...
export async function loadMemories(): Promise<Memory[]> {
  try {
    ...
    console.error(
      "[Memory] Error loading memories, returning fallback:",
      error,
    );
    return [];
  }
...
export function formatSystemInstructionsWithMemories(
  ...
  const categoriesOrdered = [
    ...
    {
      key: "preference",
      label: "Preferences & Tastes (Likes, dislikes, games, movies)",
    },
...
export async function processConversationSlice(
  ...
  console.log(
    "[Memory] Initiating pipeline for dialogue slice of length:",
  ...
  try {
    ...
    const memoryContext = currentMemories
      .map((m) => `ID: ${m.id} | Category: ${m.category} | Fact: ${m.text}`)
      .join("\n");
    const dialogueContext = dialogueHistory
      .map((line) => `${line.role === "user" ? "User" : "Myraa"}: ${line.text}`)
      .join("\n");
...
- ID: For ADD, leave blank. For UPDATE or REMOVE, provide the exact 'id' from the "Current user memories" list.`;
              ...
              items: {
                ...
                properties: {
                  ...
                  id: {
                    type: Type.STRING,
                    description:
                      "Specific ID of the existing memory being modified or deleted (leave blank/null for ADD).",
                  },
                  category: {
                    type: Type.STRING,
                  ...
                  text: {
                    type: Type.STRING,
                    description:
                      "The memory summarized as a concise declarative statement in third-person.",
                  },
                },
    ...
    if (transactions.length === 0) {
      console.log(
        "[Memory] Zero transactions generated. Ignored routine conversations.",
      );
    ...
    console.log(
      `[Memory] Processing ${transactions.length} memory updates:`,
